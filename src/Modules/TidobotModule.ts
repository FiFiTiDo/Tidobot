import AbstractModule, {Symbols} from "./AbstractModule";
import ChatterEntity from "../Database/Entities/ChatterEntity";
import Permission from "../Systems/Permissions/Permission";
import {Role} from "../Systems/Permissions/Role";
import IgnoredEntity from "../Database/Entities/IgnoredEntity";
import Command from "../Systems/Commands/Command";
import {CommandEvent} from "../Systems/Commands/CommandEvent";
import {ChatterArg} from "../Systems/Commands/Validation/Chatter";
import {getLogger} from "../Utilities/Logger";
import {command} from "../Systems/Commands/decorators";
import {permission} from "../Systems/Permissions/decorators";
import {version} from "../../package.json";
import {CommandHandler} from "../Systems/Commands/Validation/CommandHandler";
import CheckPermission from "../Systems/Commands/Validation/CheckPermission";
import {Argument, ResponseArg, Sender} from "../Systems/Commands/Validation/Argument";
import {Response} from "../Chat/Response";
import {StringArg} from "../Systems/Commands/Validation/String";

export const MODULE_INFO = {
    name: "Tidobot",
    version: "1.1.0",
    description: "Used to manage core bot functionality other than settings and get more info"
};

const logger = getLogger(MODULE_INFO.name);

class TidobotCommand extends Command {
    constructor() {
        super("tidobot", "<version|about|ban|unban>");
    }

    @CommandHandler(/^(tidobot|bot|tb) ver(sion)?/, "tidobot version", 1)
    @CheckPermission("tidobot.version")
    async version(event: CommandEvent, @ResponseArg response: Response): Promise<void> {
        return response.message("tidobot:version", {version});
    }

    @CommandHandler(/^(tidobot|bot|tb) about/, "tidobot about [target]", 1)
    @CheckPermission("tidobot.about")
    async about(
        event: CommandEvent, @ResponseArg response: Response, @Sender sender: ChatterEntity,
        @Argument(StringArg, "target", false) target: string = null
    ): Promise<void> {
        return response.message("tidobot:about", {username: target ?? sender.name});
    }

    @CommandHandler(/^(tidobot|bot|tb) ignore/, "tidobot ignore <user>", 1)
    @CheckPermission("tidobot.ignore")
    async ignore(
        event: CommandEvent, @ResponseArg response: Response, @Argument(new ChatterArg()) chatter: ChatterEntity
    ): Promise<void> {
        return IgnoredEntity.add(chatter.getService(), chatter.userId)
            .then(ignored => response.message(ignored ? "user:ignore.added" : "user:ignore.already", {username: chatter.name}))
            .catch(e => response.genericErrorAndLog(e, logger));
    }

    @CommandHandler(/^(tidobot|bot|tb) unignore/, "tidobot unignore <user>", 1)
    @CheckPermission("tidobot.unignore")
    async unignore(
        event: CommandEvent, @ResponseArg response: Response, @Argument(new ChatterArg()) chatter: ChatterEntity
    ): Promise<void> {
        return IgnoredEntity.remove(chatter.getService(), chatter.userId)
            .then(ignored => response.message(ignored ? "user:ignore.removed" : "user:ignore.not", {username: chatter.name}))
            .catch(e => response.genericErrorAndLog(e, logger));
    }

    @CommandHandler(/^(tidobot|bot|tb) ban/, "tidobot ban <user>", 1)
    @CheckPermission("tidobot.ban")
    async ban(
        event: CommandEvent, @ResponseArg response: Response, @Argument(new ChatterArg()) chatter: ChatterEntity
    ): Promise<void> {
        if (chatter.banned) return response.message("user:ban.already", {username: chatter.name});
        chatter.banned = true;
        chatter.save()
            .then(() => response.message("user:ban.added", {username: chatter.name}))
            .catch(e => response.genericErrorAndLog(e, logger));
    }

    @CommandHandler(/^(tidobot|bot|tb) unban/, "tidobot unban <user>", 1)
    @CheckPermission("tidobot.unban")
    async unban(
        event: CommandEvent, @ResponseArg response: Response, @Argument(new ChatterArg()) chatter: ChatterEntity
    ): Promise<void> {
        if (!chatter.banned) return response.message("user:ban.not", {username: chatter.name});
        chatter.banned = false;
        chatter.save()
            .then(() => response.message("user:ban.removed", {username: chatter.name}))
            .catch(e => response.genericErrorAndLog(e, logger));
    }
}

class RegularCommand extends Command {
    constructor() {
        super("regular", "<add|remove>", ["reg"]);
    }

    @CommandHandler(/^reg(ular)? add/, "regular add <user>", 1)
    @CheckPermission("regular.add")
    async addRegular(
        event: CommandEvent, @ResponseArg response: Response, @Argument(new ChatterArg()) chatter: ChatterEntity
    ): Promise<void> {
        if (chatter.regular) return response.message("user:regular.already", {username: chatter.name});
        chatter.regular = true;
        chatter.save()
            .then(() => response.message("user:regular.added", {username: chatter.name}))
            .catch(e => response.genericErrorAndLog(e, logger));
    }

    @CommandHandler(/^reg(ular)? rem(ove)?/, "regular remove <user>", 1)
    @CheckPermission("regular.remove")
    async removeRegular(
        event: CommandEvent, @ResponseArg response: Response, @Argument(new ChatterArg()) chatter: ChatterEntity
    ): Promise<void> {
        if (!chatter.regular) return response.message("user:regular.not", {username: chatter.name});
        chatter.regular = false;
        chatter.save()
            .then(() => response.message("user:regular.removed", {username: chatter.name}))
            .catch(e => response.genericErrorAndLog(e, logger));
    }
}

export default class TidobotModule extends AbstractModule {
    static [Symbols.ModuleInfo] = MODULE_INFO;
    @command tidobotCommand = new TidobotCommand();
    @command regularCommand = new RegularCommand();
    @permission viewBotVersion = new Permission("bot.version", Role.NORMAL);
    @permission viewBotInfo = new Permission("bot.about", Role.NORMAL);
    @permission addIgnored = new Permission("bot.ignore", Role.OWNER);
    @permission removeIgnored = new Permission("bot.unignore", Role.OWNER);
    @permission addBanned = new Permission("bot.ban", Role.BROADCASTER);
    @permission removeBanned = new Permission("bot.unban", Role.BROADCASTER);
    @permission addRegular = new Permission("regular.add", Role.MODERATOR);
    @permission removeRegular = new Permission("regular.remove", Role.MODERATOR);

    constructor() {
        super(TidobotModule);

        this.coreModule = true;
    }
}