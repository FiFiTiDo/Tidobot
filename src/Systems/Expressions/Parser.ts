...
export default class ExpressionParser extends CstParser {
    ...
    public dotNotation = this.RULE("dotNotation", (): void => {
        ...
        this.MANY((): void => {
            this.OR([
                {ALT: (): void => { this.SUBRULE(this.objectAccess, {LABEL: "rhs"}); }},
                {ALT: (): void => { this.SUBRULE(this.arrayAccess, {LABEL: "rhs"}); }},
                {ALT: (): void => { this.SUBRULE(this.functionCall, {LABEL: "rhs"}); }}
            ]);
        });
    });
    ...
    public valueExpression = this.RULE("valueExpression", (): void => {
        this.OR([
            ...
            {ALT: (): void => { this.CONSUME(Tokens.Boolean); }},
            {ALT: (): void => { this.SUBRULE(this.array); }},
            {ALT: (): void => { this.SUBRULE(this.dotNotation); }},
            {ALT: (): void => { this.SUBRULE(this.parenthesisExpression); }}
        ]);
    });
    public notExpression = this.RULE("notExpression", (): void => {
        this.OR([
            ...
            {ALT: (): void => { this.SUBRULE(this.valueExpression, {LABEL: "value"}); }}
        ]);
    });
