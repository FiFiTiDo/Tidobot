...
export default class MessageCache {
    private messages: ChannelStateList<Message[]> = new ChannelStateList<Message[]>([]);

    constructor() {
        const settings = SettingsSystem.getInstance();
        settings.registerSetting(new Setting("filter.cache.amount", "30", SettingType.INTEGER));
    }
    ...
    async getMaxMessages(channel: ChannelEntity): Promise<number> {
        return channel.getSetting<number>("filter.cache.amount");
    }
    ...
    getAll(channel: ChannelEntity): Message[] {
        return this.messages.getChannel(channel);
    }
